...
printEachForward(brkIter, example).map { charString =>
	val utf8Bytes = charString.getBytes(StandardCharsets.UTF_8)
	val codePoint = charString.codePointAt(0)
	val codePointBytes = intToByteArray(codePoint)
	s"$charString | ${toHexString(codePointBytes)} | ${toBinString(codePointBytes)} | ${toHexString(utf8Bytes)} | ${toBinString(utf8Bytes)}"
}.foreach(println)